import sys
from PyQt5.QtWidgets import QApplication, QWidget, QMessageBox, QPushButton, QGridLayout, QLabel, QMainWindow, QAction, \
    qApp
...
#from Workers.DeusExMultiplayerWorker import DeusExMultiplayerWorkerThread
...
def spaceshipShoot(self, spaceship: Spaceship, enemies, score_label, score_list, box: Box, hearts: list,
                   socketManager: SocketManager, spaceship_bullet_id, spaceship_bullets: list, current_lvl: int, usernameScoreIndex: int):
    print(f"////////////////////Bullet koji je stigao u SpaceshipShoot {spaceship_bullet_id}")
    self.worker = BulletWorkerMultiplayerThread(self, spaceship, enemies, score_label, score_list, box, hearts,
                                                socketManager, spaceship_bullet_id, spaceship_bullets, current_lvl, usernameScoreIndex)
    self.worker.start()
    self.worker.finished.connect(bullet_finished)
    # self.worker.user_dead.connect(self.user_died)
...
def spaceshipShootRival(self, spaceship: Spaceship, enemies, score_label, score_list, box: Box, hearts: list,
    ...
    print(f"////////////////////Bullet koji je stigao u SpaceshipShoot {spaceship_bullet_id}")
    self.rivalWorker = BulletWorkerRivalMultiplayerThread(self, spaceship, enemies, score_label, score_list, box, hearts,
    ...
    # self.rivalWorker.user_dead.connect(self.user_died)
    #self.rivalWorker.update_lifes_number.connect(self.updateNumberOfLivesLuckyFactor)
...
class Client(QMainWindow):
    ...
    def updateEnemiesBullet(self, enemy_bullet: Bullet):
        ...
        if self.collisionHandler._handleSpaceshipWithEnemyBulletCollision(self.spaceship, self.enemy_bullets):
            isDead = self.collisionHandler.updateNumberOfLives(self.hearts, self.socketManager, self.player_username, self.score_list[self.usernameScoreIndex])
            if isDead:
    ...
    def init_UI(self):
        ...
        # self.finished_winner_label.setText(f"Player {self.player_username} died!")
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Space:
            ...
            spaceshipShoot(self, self.spaceship, self.enemies, self.score_label, self.score_list, self.box, self.hearts,
                           self.socketManager, str(spaceship_bullet_counter), self.spaceship_bullets, self.level_num, self.usernameScoreIndex)
    ...
    def create_new_level(self, level_num: int):
        ...
        self.enemiesWorker = EnemiesWorkerMultiplayerThread(self.enemies, self.shields, self.level_num, self.socketManager)
        ...
        self.BulletEnemyWorkerMultiplayer = BulletEnemyWorkerMultiplayerThread(self, self.enemies, self.enemy_bullets,
                                                                               self.level_num, str(enemy_bullet_counter))
        ...
        self.newBulletEnemyWorkerMultiplayer = BulletEnemyWorkerMultiplayerThread(self, self.enemies,
                                                                                  self.enemy_bullets, self.level_num, str(enemy_bullet_counter))
        self.newBulletEnemyWorkerMultiplayer.start()
        # self.newBulletEnemyWorkerMultiplayer.finish_enemy_shooting.connect(self.finishedWithEnemyBulletWorkerHelper)
        ...
        # self.DeusExMultiplayerWorker.finish_enemy_shooting.connect(self.finishedWithEnemyBulletWorker)
    ...
    def spaceship_shoot_listen(self, spaceship_id: str, bullet_id: str):
        print(f"USER {spaceship_id} opalio metak")
        print("<33333333333" * 100)
        print(f"/////////////////////Spaceship shoot listen Spaceship_id: {spaceship_id}")
        print(f"/////////////////////Spaceship shoot listen bullet_id: {bullet_id}")
        for spaceship in self.all_spaceships:
            if spaceship.spaceship_id == spaceship_id:
                spaceshipShootRival(self, spaceship, self.enemies, self.score_label, self.score_list, self.box, self.hearts,
    ...
    def remove_enemy_listen(self, enemy_id: str, bullet_id: str):
        print("*" * 69)
        print(f"{enemy_id}")
        print(f"---------->{type(bullet_id)}<---------------------")
        print("*" * 69)

        for enemy in self.enemies:
            print(f"|||||||||||||||||||||||||||||||||{enemy.object_id[0]} == {enemy_id}")
            print(f"|||||||||||||||||||||||||||||||||{type(enemy.object_id[0])} == {type(enemy_id)}")
        ...
        for bullet in self.spaceship_bullets:
            print(f"{bullet.object_id}", end="")
            print(f"|||||||||||||||||||||||||||||||||{bullet.object_id} == {bullet_id}")
            print(f"|||||||||||||||||||||||||||||||||{bullet.object_id[0]} == {bullet_id}")
            print(f"|||||||||||||||||||||||||||||||||{type(bullet.object_id)} == {bullet_id}")
            if str(bullet.object_id[0]) == bullet_id:
                print("Usao sam u brisanje Bulleta")
                bullet.hide()
                self.spaceship_bullets.remove(bullet)
        print(f"//////////////////////////////////")

        if( len(self.enemies) == 0):
            print("++++++++++++++++++++++++++++++++++++++Treba da kreiram lvl")



        for enemy in self.enemies:
            print(enemy.object_id, end="")
# obrisati samo kada umre, tj hide dodati kada isDead = true
    ...
    def update_rival_score_listen(self, scoreIndex: int, scoreValue: int):
        ...
        #print(f"ccccccccccccccScore koji je stigao u updateRival: {scoreIndex}, valu={scoreValue}, duzina {len(self.score_list)}")
    ...
    def remove_spaceship_listen(self, spaceship_id: str):
        ...
        #print(f"qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqOvo da hide{spaceship_id}")
        for spaceship in self.all_spaceships:
            if spaceship.spaceship_id == spaceship_id:
                spaceship.hide()
